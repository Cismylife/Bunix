...
heap_t *create_heap(u32int start, u32int end_addr, u32int max, u8int supervisor, u8int readonly)
{
	...
	ordered_array_t temp	= place_ordered_array((void*)start, HILIX_HEAP_INDEX_SIZE, &header_t_less_than);
	heap->index		= temp;

	start		       += sizeof(type_t)*HILIX_HEAP_INDEX_SIZE;
...
u32int contract(u32int new_size, heap_t *heap)
{
	...
	if (new_size < HILIX_HEAP_MIN_SIZE)
	{
		new_size = HILIX_HEAP_MIN_SIZE;
	}
...
void *alloc(u32int size, u8int page_align, heap_t *heap)
{
	...
	if (iterator == -1)
	{
		...
		if (idx == -1)
		{
			...
			header->magic		= HILIX_HEAP_MAGIC;
			header->size		= new_length - old_length;
			header->is_hole		= 1;

			footer_t *footer	= (footer_t *)(old_end_address + header->size - sizeof(footer_t));
			footer->magic		= HILIX_HEAP_MAGIC;
	...
	block_header->magic		= HILIX_HEAP_MAGIC;
	block_header->is_hole		= 0;
	block_header->size		= new_size;

	footer_t *block_footer		= (footer_t *)(orig_hole_pos + sizeof(header_t) + size);
	block_footer->magic		= HILIX_HEAP_MAGIC;
...
void free(void *p, heap_t *heap)
{
	...
	ASSERT(header->magic == HILIX_HEAP_MAGIC);
	ASSERT(footer->magic == HILIX_HEAP_MAGIC);
	...
	if (test_footer->magic == HILIX_HEAP_MAGIC && test_footer->header->is_hole == 1)
	{
	...
	if (test_header->magic == HILIX_HEAP_MAGIC && test_header->is_hole)
	{
